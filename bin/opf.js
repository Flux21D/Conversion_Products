	...
	for(var f=0; f<files.length; f++){
		...
		if(stat.isDirectory()){
			...
			if(files[f] == "OEBPS"){
				...
				fs.writeFileSync(reportFolder + '\\TitleLevel_Error_Log' + filename.toString().replace(input + '\\','').replace('\\' + files[f],'') + '.txt',TitleLevelerrorLog); //Write the Title Level Error Log
